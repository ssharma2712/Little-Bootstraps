little_bootstraps <- function(data_path, candidate_tree, evo_model = NULL, lb_parameter = c(0.8,10,10), output_file = NULL, del = 0.001, precision = FALSE){
  ...
  if(precision == FALSE){
    aggregator(dirname(di), ".treefile", candidate_tree, s = lb_parameter[2], r = lb_parameter[3])
  }else{
    lb_precision(dirname(di), ".treefile", candidate_tree, s = lb_parameter[2], r = lb_parameter[2])
  }
